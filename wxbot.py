...
class WXBot:
    ...
    def __init__(self):
        ...
        self.batch_count = 150    #一次拉取50个联系人的信息
    ...
    def handle_msg(self, r):
        ...
        for msg in r['AddMsgList']:
            ...
            if self.DEBUG and msg_type_id != 0:
                ...
                if userf == 'unknown':
                    self.get_contact()
                    print u'[MSG1] %s:' % user['name']
                else:
                    print u'[MSG2] %s:' % user['name']
    ...
    def get_icon(self, uid, gid, name):
        """
        获取联系人或者群聊成员头像
        :param uid: 联系人id
        ...
        data = r.content
        fn = 'icon_' + name['name'] + '.jpg'
       
        with open(os.path.join(self.temp_pwd + '/icon',fn), 'wb+') as f:
            f.write(data)
        return fn

    def get_head_img(self, uid, nicknamed):
        """
        获取群头像
        :param uid: 群uid
        """
        ...
        r = self.session.get(url)
        data = r.content
        fn = 'head_' + nicknamed + '.jpg'
        with open(os.path.join(self.temp_pwd + '/head',fn), 'wb') as f:
            f.write(data)
        return fn
    ...
    def get_msg_img(self, msgid):
        ...
        data = r.content
        fn = 'img_' + msgid + '.jpg'
        with open(os.path.join(self.temp_pwd + '/images',fn), 'wb+') as f:
            f.write(data)
        return fn
    ...
    def get_voice(self, msgid):
        ...
        data = r.content
        fn = 'voice_' + msgid + '.mp3'
        with open(os.path.join(self.temp_pwd + '/voice',fn), 'wb') as f:
            f.write(data)
        return fn
    ...
    def get_video_url(self, msgid):
        ...
        data = r.content
        fn = 'video_' + msgid + '.mp4'
        with open(os.path.join(self.temp_pwd + '/video',fn), 'wb+') as f:
            f.write(data)
        return fn
